<template>
  <div class="h-[100dvh] flex flex-col bg-gray-50 overflow-hidden relative">
    ...
    <!-- HEADER -->
    <div class="bg-[#10499b] text-white px-4 py-4 flex items-center justify-between shadow-md pt-6 shrink-0 z-[500]">
      <div class="flex items-center gap-4">
        ...
        <h1 class="text-lg font-medium">Perencanaan Perusahaan</h1>
      </div>
    ...
    <div class="flex-1 relative z-10 bg-slate-200">
      <div id="mapPerusahaan" class="w-full h-full"></div>

      <!-- LEGENDA PETUGAS (Overlay di atas Map) -->
      <div class="absolute top-4 right-4 bg-white/95 backdrop-blur p-3 rounded-xl shadow-lg border border-gray-200 z-[400] max-w-[160px] max-h-[50vh] overflow-y-auto">
        <h4 class="text-[10px] font-bold text-gray-700 mb-2 border-b pb-1 uppercase tracking-wider">Warna Petugas</h4>
        <div class="space-y-2">
          <div v-for="p in store.petugasList" :key="p.id" class="flex items-center text-[11px]">
            <div class="w-3 h-3 rounded-full mr-2 shadow-sm border border-gray-200 shrink-0" :style="{ backgroundColor: p.warna }"></div>
            <span class="text-gray-600 truncate">{{ p.nama }}</span>
          </div>
          <div v-if="store.petugasList.length === 0" class="text-[10px] text-gray-400 italic">Belum ada petugas</div>
          <div class="flex items-center text-[11px] mt-2 pt-2 border-t border-gray-100">
            <div class="w-3 h-3 rounded-full mr-2 shadow-sm border border-gray-200 shrink-0 bg-slate-400"></div>
            <span class="text-gray-500 italic">Belum Diassign</span>
          </div>
        </div>
      </div>
    </div>
      ...
      <div v-if="selectedLokasi" class="p-5">
        ...
        <div class="flex justify-between items-start mb-4">
          <div>
            <div class="flex items-center gap-1 mb-1">
              <span class="text-[9px] font-bold uppercase px-1.5 py-0.5 rounded bg-blue-100 text-blue-700">
                Perusahaan
              </span>
            </div>
        ...
        <div class="bg-blue-50 border border-blue-100 rounded-xl p-4 mb-4">
          ...
          <select
            ...
            class="w-full text-sm p-2.5 border border-blue-200 rounded-lg outline-none focus:ring-2 focus:ring-blue-500 bg-white font-medium"
          >
      ...
      <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md flex flex-col overflow-hidden animate-slide-up">
        ...
        <div class="p-4 bg-white border-b border-gray-100">
          <form @submit.prevent="tambahPetugas" class="flex gap-2">
            <input v-model="formPetugas.nama" type="text" required placeholder="Nama Petugas" class="flex-1 border border-gray-300 p-2.5 rounded-lg text-sm outline-none focus:ring-2 focus:ring-blue-500">
            <input v-model="formPetugas.warna" type="color" class="h-[42px] w-12 border border-gray-300 rounded-lg cursor-pointer p-0.5">
            <button type="submit" class="bg-[#10499b] text-white px-4 rounded-lg text-sm font-bold active:scale-95 transition">Tambah</button>
          </form>
        </div>
      ...
      <div class="bg-gray-100 rounded-2xl shadow-2xl w-full max-w-4xl h-[90vh] flex flex-col overflow-hidden animate-slide-up">
        ...
        <div class="p-4 border-b border-teal-700 flex justify-between items-center bg-teal-600 text-white shrink-0">
          <h3 class="font-bold flex items-center gap-2">
            <span class="material-symbols-outlined">description</span> Pratinjau Penempatan (Perusahaan)
          </h3>
          ...
          <div v-else class="max-w-4xl mx-auto space-y-6">
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
              <div class="bg-white p-4 rounded-xl shadow-sm border border-gray-200 text-center flex flex-col justify-center">
                <span class="text-[10px] font-bold text-gray-500 uppercase tracking-wider">Total Perusahaan</span>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div v-for="(group, pName) in reportData.grouped" :key="pName" class="bg-white rounded-xl shadow-sm border border-gray-200 flex flex-col overflow-hidden max-h-[350px]">
                <div class="bg-gray-50 px-4 py-3 border-b border-gray-200 flex justify-between items-center shrink-0">
                  ...
                  <span class="bg-blue-100 text-blue-700 text-[10px] font-bold px-2 py-1 rounded-md shrink-0">{{ group.list.length }} Lokasi</span>
                </div>
...
const filteredLokasiMap = computed(() => {
  return store.lokasiList.filter(loc => {
    const kat = loc.kategori || 'Perusahaan'
    return kat === 'Perusahaan'
  })
})
...
function initMap() {
  map = L.map('mapPerusahaan', { zoomControl: false }).setView([-6.27, 107.14], 12)
...
async function hapusPetugas(p) {
  if(confirm(`Hapus petugas ${p.nama}? Perusahaan yang diassign ke dia akan menjadi kosong.`)) {
...
function cetakLaporan() {
  ...
  let html = `
    <div style="text-align: center; border-bottom: 2px solid #000; padding-bottom: 15px; margin-bottom: 20px;">
      ...
      <h3 style="font-size: 14px; font-weight: bold; text-transform: uppercase; margin: 5px 0 0 0; color: #000;">Data Rute Perusahaan</h3>
    ...
    <table style="width: 100%; border-collapse: collapse; border: 1px solid #000; margin-bottom: 25px; background-color: #f9fafb;">
      <tr>
        <td style="width: 25%; text-align: center; border-right: 1px solid #ccc; padding: 10px;">
          <div style="font-size: 10px; font-weight: bold; text-transform: uppercase; color: #555;">Total Perusahaan</div>
  ...
  doc.write(`
    <html>
    <head>
      <title>Laporan_Penempatan_Petugas_Perusahaan</title>
      <style>
...
/* Fix untuk Leaflet di Vue/Tailwind */
:deep(.leaflet-container) { z-index: 10 !important; }

/* Custom CSS untuk Label Perusahaan di Map */
